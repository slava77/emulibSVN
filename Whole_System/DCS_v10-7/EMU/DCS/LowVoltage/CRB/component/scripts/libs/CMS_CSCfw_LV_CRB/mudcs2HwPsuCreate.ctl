...
string s1,s2;  
   ...
   for(i=1;i<=dynlen(ds1);i++){   
    ...
    if(CSC_fwCAN1_g_NEW_ELMB_SWITCH)s1+=".value";
    dpSet(s1,EPSU_POWER_ON_VALUE);
...
dyn_int BranchesPerCrate; // bracnches per crate (i.e. per elmb)

if(CSC_fwCAN1_g_DEV_MODE){// corr++
BranchesPerCrate=makeDynInt(1); // bracnches per crate (i.e. per elmb)
}
else{
BranchesPerCrate=makeDynInt(4); // bracnches per crate (i.e. per elmb)
}
