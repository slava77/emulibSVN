...
string fwFsm_getLogicalDeviceName(string pdev)
{
 ...
 string logical=mudcsGetLogical(pdev); // mycorr++
 if(logical!="")return logical;        // mycorr++
...
string fwFsm_getPhysicalDeviceName(string ldev)
{
 ...
 string phys=mudcsGetPhysical(ldev); // mycorr++
 if(phys!="")return phys;        // mycorr++
...
int fwFsm_isObjectReference(string obj)
{
	...
	fwTree_getNodeCU(sys+":"+obj, cu, exInfo);
...
fwUi_informUser(string msg, int x, int y, string ok="", string addText = "")
{
	ChildPanelOn("fwTreeDisplay/myMessageInfo.pnl","Info",
		makeDynString(msg, ok, addText),x,y);
}
