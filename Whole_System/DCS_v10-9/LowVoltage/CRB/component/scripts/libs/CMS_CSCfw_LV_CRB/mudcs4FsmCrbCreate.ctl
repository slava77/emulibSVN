...
//===========================================
 if(!CSC_fwCAN1_g_DEV_MODE){ // corr++ 
  ...
  for(iStationCross=20;iStationCross<=20;iStationCross++){ 
   mudcsCrbFsmCreatePC(iStationCross, CSC_fwCAN1_g_csc_part);
  } // for iStationCross   
 }
...
CSC_fwCAN1_g_EmuCmsGlobalNode="CSC_ME_"+emu_system_side+"_LV_CRB";
}
else {
...
if(iStationCross==20){iStation=20;emu_system_side="DEV";} //corr+  
...
string dpN;
//---------------------------------------
string parent_domain, parent_domain2;
string Node_save;
int cu_flag;
string parent_node, parent_node_2;
...
//  dpGet(mudcsLvAddSystem("db_mrtn.list"),list);
  dpGet(mudcsLvAddSystem("db_pcmb.list"),list);
  list[2]=list[1];
  strreplace(list[1],"_PSU","_PLUS_PSU");  
  strreplace(list[2],"_PSU","_MINUS_PSU");
//---------------------
parent_domain=system_parent_node;
...
//-------------------
 ...
 for(j=1;j<=dynlen(ds2);j++){
 if(strpos(ds2[j],"Branch00")>=0 || strpos(ds2[j],"Branch01")>=0){
   if(strpos(parent_domain2,"PLUS")<0)continue;
 }
 else{
   if(strpos(parent_domain2,"MINUS")<0)continue;
 }
...
// CSC_fwCAN1_g_EmuCmsGlobalNode= "CSC"+station_label+emu_system_side+idisk+"p"+i+"CRB_1"; 
 Node_save=CSC_fwCAN1_g_EmuCmsGlobalNode;
 CSC_fwCAN1_g_EmuCmsGlobalCu="0";cu_flag = 0;
 CSC_fwCAN1_g_EmuCmsGlobalParent=parent_domain2;
