...
DebugTN(fsm2+" --------------------------+ "+current_error_status+" "+ds_count[i]);                      
 ...
 DebugTN(fsm+" -------------------------- "+current_error_status+" "+ds_count[i]);
        ...
        dyn_string ds_alerts; 
        dyn_int alert_channels;    
        mudcsCrbGetDmbTempAlerts(fsm, ds_alerts, alert_channels);
...
//      DebugTN(">>>> "+remote_system+":LV_CONFIRMATION_SERVICE.");
...
DebugTN("5:"+update_fun_status+" "+sPowerDpe);        
        while(CSC_fwCAN1_g_BLOCK_NEXT_DPCONNECT);
