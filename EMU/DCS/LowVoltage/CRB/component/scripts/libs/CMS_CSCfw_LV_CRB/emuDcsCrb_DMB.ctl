...
CSC_fwCAN1_g_P5_SIM=true;
     
addGlobal("CSC_fwCAN1_g_DEV_MODE", BOOL_VAR); // corr++
 string hostname1=getHostname();
  if((hostname1=="EMU-DCS-DEV1" || hostname1=="emu-dcs-dev1") && !CSC_fwCAN1_g_P5_SIM){ //  DEV machine  // corr++
   CSC_fwCAN1_g_DEV_MODE=true;  
  }
  else{
   CSC_fwCAN1_g_DEV_MODE=false;      
  }
...
//====================================================
mudcsCrbGetDmbTempAlerts(string DpName, dyn_string &ds_alerts, dyn_int &alert_channels){
 
   int i,p1,p2;
  string dp_template, ds_alerts_notordered, test_string;
  dyn_string dps;
  dyn_float temps;
  string alertClass;
 ...
 for(i=1;i<=dynlen(ds_alerts_notordered);i++){
  if(strpos(dp_template,"PC6")>=0 && dynlen(dp_template)==6){
