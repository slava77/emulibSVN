...
// so the code below covers that
  ...
  if (globalExists("CSC_fwCAN1_g_SYSTEM_NAME")) {
    s1 = CSC_fwCAN1_g_SYSTEM_NAME;
...
///  DebugN("mudcsSetAliases: +++++++++++"+emu_aliases[i]);
    ...
    if (pair[2] == logical) {
      DebugN("mudcsGetPhysical ++++++++++++++++++++++ " + pair[2] + " phys=" + pair[1]);
...
//if(dynlen(emu_aliases)==0)emu_aliases=CSC_fwG_g_EMU_ALIASES;
  ...
  for (i = 1; i <= dynlen(emu_aliases); i++) {
    ...
    if (strpos(pair[1], phys) >= 0) {
      DebugN("mudcsGetLogical ++++++++++++++++++++++ " + pair[2] + " phys=" + pair[1]);
      return pair[2];
    }
  }
