...
main(){
  ...
  int i;
  for(i=1;i<=20;i++){DebugTN("START");}
    
  mudcsCrbPnlInit();
  mudcsCrbInitDelays(0); // additional for the server
  //==============
  all_crb_dps = dpNames("*","fwCrb_CSC_LV");
  DebugTN("dpNames(*,fwCrb_CSC_LV)");
  DebugTN(all_crb_dps);
  for(i=1;i<=dynlen(all_crb_dps);i++){  
    crb_busy[i]=false;
  }
...
dist_connections_detect(string dpName, dyn_int dists){
  ...
  //if(CSC_fwCAN1_g_PLUS_SYSTEM_NAME==""||CSC_fwCAN1_g_MINUS_SYSTEM_NAME==""){
    ...
    delay(15);
    mudcsCrbPnlInit();
    DebugTN("_CSC_fwCAN1_g_PLUS_SYSTEM_NAME="+CSC_fwCAN1_g_PLUS_SYSTEM_NAME);
    DebugTN("_CSC_fwCAN1_g_MINUS_SYSTEM_NAME="+CSC_fwCAN1_g_MINUS_SYSTEM_NAME);
...
alert_config_changed_dp(string dpName, string value){
  ...
  string update_fun_alert="mudcsCrbUpdateMonitor_"+"fwCrb_CSC_LV"+"_alert";
  dpConnect(update_fun_alert/*,false*/,value+".:_alert_hdl.._act_state_color");
...
mudcsCrb_threadWatchForTimeouts(){
 ...
 dyn_int ds_count, ds_count_1, ds_count_error1,ds_count_error2,ds_count_error3, ds_elmb_hang;
 int max_guard=0;
 bool FIRST=true;
 int general_count=0;
 int count_opc_on=0;
...
DebugTN("mudcsCrb_threadWatchForTimeouts"); 

 //continue;
   
//========================================

if(/*two_way_communications_x2p &&*/ automatic_stop_pcrate_slow_control){  // x2p
...
//==========================================   
   ...
   general_count++;
   if(ServerState==1)count_opc_on++;
      
   DebugTN("GUARDING WITH INTERVAL="+max_guard+" general_count="+general_count);
   for(i=1;i<=dynlen(ds1);i++){
...
//     status_complex=0;
     ...
     status_complex=0;
     if(status != 5 && status != 133){ if(ServerState==1)ds_count_error2[i]++;  
                                          status_complex |= 2048;} // bit 1 is asserted
     ...
     if(FIRST){
       ...
       if(current_elmb_state_update-previous_elmb_state_update[i] < 2) { // elmb is frozen
          if(ServerState==1)ds_count_error1[i]++;
...
DebugTN(fsm2+" --------------------------+ "+current_error_status+" "+ds_count[i]);                      
 ...
 DebugTN(fsm+" -------------------------- "+current_error_status+" "+ds_count[i]);
        ...
        if(dynlen(alert_channels)>0)status_complex |= 8192;
   ...
   dpSetWait("ELMB_ERRORS.",ELMB_ERRORS);
   dpSetWait("GENERAL_COUNT.",general_count);
   dpSetWait("COUNT_OPC_ON.",count_opc_on);      
 } // while(1)
}
//========================================================
...
//----- x2p
 ...
 if(two_way_communications_x2p){
  ...
  for(int i=1;i<=2;i++){
   ...
   //dpConnect("update_confirmation",FALSE,remote_system+":LV_1_COM_CONF.command");
   if(remote_system!=""){
    if((i==1 && !connected_minus_side) || (i==2 && !connected_plus_side)){ 
     dpConnect("update_confirmation",FALSE,remote_system+":LV_CONFIRMATION_SERVICE.value");
     update_confirmation(remote_system+":LV_CONFIRMATION_SERVICE.value", "SOFT_START"); // init minus    
      if(i==1)connected_minus_side=true;
      else if(i==2) connected_plus_side=true;      
    }
   }
  } // for      
  
//   update_confirmation("CSC_fwCAN1_g_MINUS_SYSTEM_NAME:", "SOFT_START"); // init minus   
//   update_confirmation("CSC_fwCAN1_g_PLUS_SYSTEM_NAME:", "SOFT_START");  // init plus  
 } 
  */
//-----      
...
//continue;       
  ...
  if(CSC_fwCAN1_g_NEW_ELMB_SWITCH)sPowerDpeShort=sPowerDpe;
  else sPowerDpeShort= substr(sPowerDpe,0,strpos(sPowerDpe,"."));          
       dynAppend(CSC_fwCAN1_g_STATUS_HANDLERS_FIRST,mudcsLvAddSystem(sPowerDpeShort));
       dynAppend(CSC_fwCAN1_g_FULL_DPLIST,datapoints[i]);

        CSC_fwCAN1_g_BLOCK_NEXT_DPCONNECT=1;
//        dpConnect(update_fun_status,datapoints[i]+".bus");
        dynAppend(psu_power_dps,mudcsLvAddSystem(sPowerDpeShort+".value"));
        dynAppend(psu_branch_dps,mudcsLvAddSystem(datapoints[i]));        
        dpConnect(update_fun_status,sPowerDpeShort+".value");
DebugTN("5:"+update_fun_status+" "+sPowerDpeShort);        
        while(CSC_fwCAN1_g_BLOCK_NEXT_DPCONNECT);
...
mudcsCrbUpdate_single_crb_switch(string dpName, int value){
...
if(index_busy<=0){ 
  for(i=1;i<=10;i++)DebugTN("ERROR : NOT POSSIBLE SITUATION "+fsm);
  DebugTN(all_crb_dps);
  return;
}
...
//if(value == 2 || (value==-1 && index >=1) ){  // -1, -2  is to initialize the state at start  
 //two_way_communications_x2p
 if( ( (value == 2 || (value==-1 && index >=1) ) && !two_way_communications_x2p)
   ||( (value == 2 && index <1) && two_way_communications_x2p)  ){ 
  
///    dynAppend(d_s2,"11111"+dpName);
///    dpSetWait("dyn_debug2.",d_s2);

 // mudcsCrbCommandCrb(fsm,true);  MOVED TO THREAD !!!
...
dpGet(fsm+".:_alert_hdl.._active",bIsActive);
 ...
 // mudcsCrbCommandCrb(fsm,false); /// MOVED TO THREAD !!!
...
if(bIsActive){
 ...
 if(value == "FwAlarmErrorAck"){
  if(current_status != -1)dpSet(fsm+".status",-1);  
   ...
   if(current_status != 2 && current_status != 22)dpSet(fsm+".status",22);// 2
 }  
} // if(bIsActive)
else{ // !bIsActive
...
//=============================================================================================
mudcsCrb_threadCommandCrb(string fsm, bool isOn){
 mudcsCrbCommandCrb(fsm,isOn);
  
}
//=============================================================================================

mudcsCrb_threadAlertActiveSet(string dpe, bool isActive, int index_busy){
  int i;
  string fsm1=dpe;
  dpe=dpe+".:_alert_hdl.._active";
  
  if(index_busy != 0){
    crb_busy[index_busy]=true; // to block the next switching !!!
    mudcsCrbCommandCrb(fsm1,isActive);
  }
       for(i=0;i<200;i++)
       ...
       if(index_busy != 0)crb_busy[index_busy]=false;
}
//=============================================================================================
//=============================================================================================
//=============================================================================================
...
string sElmb, sBit, sBit_id, sPsu, sPsu_bit;
dyn_string argdsExceptionInfo, split_id;
time time_do_bytes;
time time_do_bit;
int time_difference;
...
sElmb=substr(dpName,0,strpos(dpName,"/DO/"));  
sBit_id=substr(dpName,0,strpos(dpName,".value"))+".id";
dpGet(sBit_id,sBit_id);
sPsu=sElmb;
strreplace(sPsu,"/","_");
split_id=strsplit(sBit_id,";");
sPsu_bit=sPsu+".bit_"+split_id[2];

string dp_byte = substr(dpName,0,strpos(dpName,"/do_"))+"/do_Bytes.do_write";
int iBit=split_id[2];
delay(0,100*iBit);

//int iBytes=1 << iBit;
unsigned uBits= fwElmbUser_CSC_getDoBits(sElmb,sBit_id, value, argdsExceptionInfo);
//unsigned uBits = fwElmbUser_getDoBytes(sElmb, argdsExceptionInfo);
...
if(CSC_fwCAN1_g_NEW_ELMB_SWITCH){ 
 
//dpGet(sElmb+"/DO/do_Bytes.do_write:_online.._stime",time_do_bytes); 
//dpGet(sPowerDpe+":_online.._stime",time_do_bit); 
time_do_bytes=getCurrentTime();
dpGet(sPsu_bit+":_online.._stime",time_do_bit);
time_difference=time_do_bytes-time_do_bit;
 if(time_difference<2){
//  CSC_fwCAN1_g_BLOCK_NEXT_DPCONNECT=0;
   isBreak=true;
  DebugTN("deadloop break");
//dpGet(fsm+".bus",bus);
//dpSetWait(fsm+".bus",bus);  
  return;

 }
 else dpSetWait(sPsu_bit,value);
...
///    dpSetWait("dyn_debug2.",d_s2);
      ...
      if((index >=1 || CSC_fwCAN1_g_NEW_ELMB_SWITCH) && !isBreak){
DebugTN(sPowerDpe+"||||||||||||||||||||||||||");
DebugTN("* "+sElmb+" > "+sBit_id+" > "+CSC_fwCAN1_g_NEW_ELMB_SWITCH);

        if(CSC_fwCAN1_g_NEW_ELMB_SWITCH){
        // fwElmbUser_setDoBit(sElmb,sBit_id,EPSU_POWER_ON_VALUE,argdsExceptionInfo);
          DebugTN("+++++ "+dp_byte);
          DebugTN(uBits);
          dpSetWait(dp_byte,uBits);
        }
        else dpSet(sPowerDpe,EPSU_POWER_ON_VALUE);         
      }
// commented otherwise indefinit loop       dpSetWait(DpName,FALSE); // inversed and is used only at the project start
       startThread("mudcsCrb_threadAlertActiveSet",fsm,true, 0/* 0 for PSU*/);
...
///mudcsDebug("updated");
 ...
 if(!dynContains(panels,fwUiPnl) || !FIRST_FSM_PANELS){
   ...
   DebugTN("FSM PANELS ARE SET");
     mudcsLvCrbFsmPanelsAndFsmAliasesSet("");
 }
...
//============================================
/*
unsigned fwElmbUser_CSC_getDoBits(string argsElmb,
                          string argdsBitId,
                          bool argdbValue,
                          dyn_string &argdsExceptionInfo)
{
  
dyn_string argdsBitIds=makeDynString(argdsBitId);  
dyn_bool argdbValues=makeDynBool(argdbValue); 
...
// ---------------
 ...
 dyn_string dsPsu=dpNames("*","FwElmbPSUBranch");  
 dynSortAsc(dsPsu);
 //dyn_bool dbPower;
 string sPower;
 bool bPower;
 for(i=1;i<=dynlen(dsPsu);i++){
   dpGet(dsPsu[i]+".power",sPower);
   dpGet(sPower+".value",bPower);
 //  dynAppend(dbPower,bPower);
   uMask  = 1 << (i-1); 
   if(bPower) uValueToSet |= uMask;
   else  uValueToSet &= ~uMask;
 }
