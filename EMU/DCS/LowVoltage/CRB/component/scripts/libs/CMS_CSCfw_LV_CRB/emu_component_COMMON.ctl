...
string dir_psu="CMS_CSCfw_LV_"+key+"\\fwElmbPSU";
...
if(key=="CRB"){
 system("cmd /c mkdir "+ProjectHome+"\\"+LV_PROJECT+"\\panels\\"+dir_psu);
 system("cmd /c mkdir "+ProjectHome+"\\"+LV_PROJECT+"\\panels\\"+dir_psu+"\\objects");
}
if(mode==2){
...
if(key=="MRTN")
 ...
 system("cmd /c cp -r "+ProjectHome+"\\panels/*"+
          " "+ProjectHome+"\\"+LV_PROJECT+"\\panels/.");  
  
 if(key=="CRB"){
  system("cmd /c cp "+ProjectHome+"\\panels\\"+dir_psu+"/*.pnl"+
          " "+ProjectHome+"\\"+LV_PROJECT+"\\panels\\"+dir_psu+"\\"+".");   
  system("cmd /c cp "+ProjectHome+"\\panels\\"+dir_psu+"\\objects/*.pnl"+
          " "+ProjectHome+"\\"+LV_PROJECT+"\\panels\\"+dir_psu+"\\objects\\"+".");     
 }
...
if(mode==2){
 system("cmd /c cp -r "+ProjectHome+"\\source\\"+dir+"\\special_stuff/* "+ProjectHome+"\\"+LV_PROJECT+"\\source\\"+dir+"\\special_stuff/.");
}
//===================================================================================================================
//==================================================================================================================
//============= XML ================================================================================================
...
command="cmd /c PVSS00ascii -out "+ProjectHome+"\\"+LV_PROJECT+"\\dplist\\"+dir+"\\"+dir+"_MINI.dpl -filterDpType _FwFsmObjectType ";
 ...
 for(i=1;i<=dynlen(dps);i++)command += " -filterDp "+dps[i];
   ...
   dps=dpNames("*","MUDCS_INT");

   command += " -filterDpType MUDCS_INT";
 for(i=1;i<=dynlen(dps);i++)command += " -filterDp "+dps[i];
