...
// ---------------
  ...
  if (synched) {
    fwElmbUser_setDoBits(argsElmb, dsBitIds, dbValues, argdsExceptionInfo, true);  
  }
  else {
    fwElmbUser_setDoBits(argsElmb, dsBitIds, dbValues, argdsExceptionInfo, false);      
  }
...
// ---------------
  // Use of mutex to lock any other DO actions on this ELMB during executiuon time
	if (synched) {
		if (!fwElmb_initLock(lockName)) {
			DebugTN("fwElmb - Asynchroneous DO set operation is requested for more than 12 ELMBs. This is not supported. Use the fwElmbUser_setDoBitsSynchronized() function instead (possible performance drop.");
			return;
		}
		fwElmb_lock(lockName);
	}
