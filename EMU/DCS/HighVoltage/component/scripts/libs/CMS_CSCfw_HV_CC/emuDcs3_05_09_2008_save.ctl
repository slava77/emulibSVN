...
//================= DB ============================================================
 ...
 dpGet(mudcsAddSystem(CSC_fwG_g_SYSTEM_NAME+":Db_o"+".CscLevelDevicesCoordinates"), CscLevelDevices);
  for(i_db=1;i_db<=dynlen(CscLevelDevices);i_db++){
   ...
   if(idisk==-1){
     emu_db=dpNames(CSC_fwG_g_MRTN_SYSTEM_NAME+":*_CR*_LV_MRTN","_FwFsmObject");      
   }
   else{ 
    if(isWholeMrtnFolderAdd){
      if(idisk==3) // for stations 2 and 3 are the same and are contained only in station 2 folder 
      emu_db=dpNames(CSC_fwG_g_MRTN_SYSTEM_NAME+":*"+EMU_SYSTEM_SIDE+"2_LV_MRTN","_FwFsmObject"); // whole folder with maratons on this disk!!!
      else emu_db=dpNames(CSC_fwG_g_MRTN_SYSTEM_NAME+":*"+EMU_SYSTEM_SIDE+idisk+"_LV_MRTN","_FwFsmObject"); // whole folder with maratons on this disk!!! 
    }
    else {
      if(idisk==3) // for stations 2 and 3 are the same and are contained only in station 2 folder 
      emu_db=dpNames(CSC_fwG_g_MRTN_SYSTEM_NAME+":*"+EMU_SYSTEM_SIDE+"2_CR*_LV_MRTN","_FwFsmObject");
      else emu_db=dpNames(CSC_fwG_g_MRTN_SYSTEM_NAME+":*"+EMU_SYSTEM_SIDE+idisk+"_CR*_LV_MRTN","_FwFsmObject");
    } 
   }
  for(i_db=1;i_db<=dynlen(emu_db);i_db++){
   s_split=strsplit(emu_db[i_db],"|");
   emu_db[i_db]=CSC_fwG_g_MRTN_SYSTEM_NAME+":"+CSC_fwG_g_MRTN_DOMAIN+"::"+s_split[dynlen(s_split)];
  } // for
 }
...
CSC_fwG_EmuCmsGlobalType=CSC_fwG_g_NodeLogicalFsmType;
mudcsNameCompose("LV_MRTN", "", "", 0, "", "", EmuCmsGlobalNode);
 parent_node=EmuCmsGlobalNode;
//EmuCmsGlobalNode="CSC"+"LV_MRTN"; //r2
CSC_fwG_EmuCmsGlobalCu="0";cu_flag = 0; // logical unit
CSC_fwG_EmuCmsGlobalParent=parent_domain;
...
//---------------------------------------------------------------------------------------------
 ...
 for(i=1;i<=dynlen(CSC_fwG_g_all_maratons);i++){ // if idisk!=-1 - only one item : the whole folder with maratons on this disk !!!   
   if(idisk==-1 || !isWholeMrtnFolderAdd)mudcs_selectParent(-1, -1, parent_node,"",-1, parent_domain);
