...
const string EMU_FSM_TYPE_LV = "EMU_LV";
const string EMU_FSM_TYPE_MARATONS = "CSC_LV_TREE_NODES";
const string EMU_FSM_TYPE_CRBS = "CSC_LV_TREE_NODES";
...
void emuFsm_createStationsFsm(string systemNode, dyn_mapping chambers, dyn_string &ex) {
  for (int i=1; i <= dynlen(chambers); i++) {
    ...
    emuFsm_createRefNode(maratonNode, stationNode, EMU_FSM_DOMAIN_LV_MRTN, EMU_FSM_TYPE_MARATONS, ex);
    if (emu_checkException(ex)) { return; }
    emuFsm_createRefNode(crbNode, stationNode, EMU_FSM_DOMAIN_LV_CRB, EMU_FSM_TYPE_CRBS, ex);
    if (emu_checkException(ex)) { return; }
    ...
    if (!emuDev_isMe11Chamber(chamber)) { // UF/PNPI HV system
      ...
      emuFsm_createRefNode(me11HvNode, chamberNode, EMU_FSM_DOMAIN_ME11_HV, EMU_FSM_TYPE_ME11_HV, ex);
      if (emu_checkException(ex)) { return; }
    }
...
void emuFsm_createHvFsm(string systemNode, string side, dyn_string primaryDps, dyn_string hvLvDps, dyn_string &ex) {
  ...
  for (int i=1; i <= dynlen(hvLvDps); i++) {
    ...
    if (dpExists(wienerDp)) {
      ...
      fwFsmTree_addNode(hvNode, hvLvNode, EMU_FSM_TYPE_LV, true);
      fwFsmTree_setNodeLabel(hvLvNode, hvLvName);
      
      // add the devices
      string wienerName = "Wiener PSU for HV " + sideFull;
      fwFsmTree_addNode(hvLvNode, wienerDp, "FwWienerMarathon", false);
      ...
      for (int i=1; i <= dynlen(channelDps); i++) {
        ...
        fwFsmTree_addNode(hvLvNode, channelDp, "FwWienerMarathonChannel", false);
  ...
  emuFsm_createRefNode(me11HvNode, hvNode, EMU_FSM_DOMAIN_ME11_HV, EMU_FSM_TYPE_ME11_HV, ex);
  if (emu_checkException(ex)) { return; }
}
...
/**
  ...
  */
void emuFsm_createRefNode(string nodeName, string parentNode, string remoteDomain, string type, dyn_string &ex) {
  if (fwFsmTree_isNode(nodeName)) { // is the node is reachable?
    if (!fwFsmTree_isNode(parentNode + "::" + nodeName)) { // if already exists locally - skip
      fwFsmTree_addNode(parentNode, remoteDomain + "::" + nodeName, type, false);
    }
  } else {
