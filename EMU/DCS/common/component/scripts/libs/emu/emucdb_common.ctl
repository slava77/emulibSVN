...
void emucdb_initialize() {
  ...
  rdbOpenConnection(g_emucdb_dbConnectionString, g_emucdb_dbConn);
  if (rdbCheckError(err, g_emucdb_dbConn)){emu_error(makeDynString("ERROR WHILE CONNECTING TO THE EMU CONFIGURATION DATABASE",err));return;};
...
dbCommand _emucdb_prepareStatement(string sql, dyn_string &exceptionInfo) {
  ...
  rdbStartCommand(g_emucdb_dbConn, sql, cmd);
