...
void hvcardx_watch_timer(unsigned long data)
{
//  return;  //!!! Remove after tests
  ...
  if (!busy) // --> Busy check
    {
              ...
              for (j=0; j<16; j++)
                {
                  ...
                  ioaddr = (ULONG)(hostcard[i]->adcdata+n+(0xF-j)*MODULE_MAX_CHANS*16);
                  iface_noise = 0;
                                      ...
                                      if (m->chan[k].imon < (imax + 500) )
                                        {
                                          ...
                                          printk(KERN_INFO PFX "%s ID#%03i at %i:%X CURRENT TRIP in Ch#%d Vtrip=%d Itrip=%d Period=%dms (hist%d, pos%ld)\n",
      ...
      // for (i=0; i<hvcard_pcie_found; i++)  hvcardx_read_adc_data(i);
  ...
  // for (i=0; i<hvcard_pcie_found; i++)  hvcardx_read_adc_data(i);
  // if (cancel_delayed_work(&dma_adc_read_work) != 0)
  {
  ...
  /*
    ...
    {
      printk(KERN_ERR PFX "Work is already running\n");
    }
  */
