...
int hvcardx_exec_cmd(ULONG pusercmd)
{
  ...
  if (err > 0)
    {
      printk(KERN_ERR PFX "Unable to copy %i bytes from user space\n", err);
      return -EFAULT;
    }
      ...
      for (card =0; card < hvcard_pcie_found; card++)
        {
          ...
          hvcardx_read_adc_data(i);
          udelay(20000);
          hvcardx_init_modules(hostcard[i]);
        }
      busy = 0;
    }
          ...
          if (cmd.size == itr_card.len*sizeof(struct HVhostcard))
            {
              ...
              if (err > 0)
                {
                  printk(KERN_ERR PFX "Unable to copy %i bytes to user space\n", err);
                  res = -EFAULT;
                }
                          ...
                          if (val != hostcard[ncard]->link[line].module[j].type)
                            {
                              ...
                              hvcardx_init_one_module(&(hostcard[ncard]->link[line].module[j]));
                            }
                          res = 0;
                        }
                      spin_unlock_irqrestore(&hvdata_lock, flags);
