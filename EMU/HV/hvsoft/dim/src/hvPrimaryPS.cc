...
int ComWrt(int port, char *buf, int size)
{
  ...
  if (port>=MAX_COM_PORTS)
    {
      std::cerr << "InitCOM: Port number is out of supported range " << MAX_COM_PORTS << std::endl;
      return res;
    }
  /*
    if (comfd[port] >= 0) {
...
int ResetHV(int port, int addr)
{
  ...
  if (SendHVCmd(port, addr, "RST")<0)
    {
      if (debug) std::cout << "ResetHV: Failed to send RST command" << std::endl;
      return -1;
    }
...
int CloseCOM(int port)
{
  ...
  if (port>=MAX_COM_PORTS)
    {
      std::cerr << "CloseCOM: Port number is out of supported range " << MAX_COM_PORTS << std::endl;
      return res;
    }
...
int EnableHVOutput(int port, int addr)
{
  ...
  if (SendHVCmd(port, addr, "SW1")<0)
    {
      if (debug) std::cout << "EnableHVOutput: Failed to send SW1 command" << std::endl;
      return -1;
    }
  return 0;
}
...
int DisableHVOutput(int port, int addr)
{
  ...
  if (SendHVCmd(port, addr, "SW0")<0)
    {
      if (debug) std::cout << "DisableHVOutput: Failed to send SW0 command" << std::endl;
      return -1;
    }
...
int GetOutputSetting(int port, int addr, unsigned int* output)
{
  ...
  if (SendHVCmd(port, addr, "SW?") <0)
    {
      if (debug) std::cout << "GetOutputSetting: Failed to send SW? command" << std::endl;
      return -1;
    }
  if (ComRdTerm(port, buf, sizeof(buf), 13) < 0)
    {
      if (debug) std::cout << "GetOutputSetting: Failed to read data-> " << buf << std::endl;
      return -1;
    }
  sbuf =  strstr (buf,"SW");
  if (sbuf && (sscanf(sbuf+2,"%d", &val) >0))
...
int GetOutputStatus(int port, int addr, unsigned int* outstatus)
{
  ...
  if (SendHVCmd(port, addr, "STS") <0)
    {
      if (debug) std::cout << "GetOutputStatus: Failed to send STS command" << std::endl;
      return -1;
    }
  usleep(10000);
  if (ComRdTerm(port, buf, sizeof(buf), 13) < 0)
    {
      if (debug) std::cout << "GetOutputStatus: Failed to read data-> " << buf << std::endl;
      return -1;
    }
  // printf("%s", buf);
  // *outstatus = 0;
