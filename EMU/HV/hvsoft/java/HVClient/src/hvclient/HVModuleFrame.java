...
//  BorderLayout borderLayout1 = new BorderLayout();
  ...
  public HVModuleFrame(int host, int card, int busaddr) {
  ...
  private void create() {
    ...
    openFrameCount++;
    try {
      hvRpcData = new DimRpcInfo("HV_DIM_SERVER_HID" + HostID + "/HV_REAL_DATA");
      hvRpcCmd = new DimRpcInfo("HV_DIM_SERVER_HID" + HostID + "/HV_REAL_CMD");
      data = new HVboard();
      jbInit();
    ...
    int UpdateRealHVData() {
      ...
      if ( (res = hvRpcData.setData(cmd.encode())) > 0 &&
          (hvRpcData.getSize() == data.getSize())) {
          // System.out.println(hvRpcData.getSize()+ " " + data.getSize());

        data.decode(hvRpcData.getData());
        UpdateHVDataWindow();
      }
      else {
    ...
    void UpdateHVDataWindow() {
      ...
      if (data.getSize() > 0) {
        ...
        infoHostID.setText("HostID: " +
                           Integer.toString(data.card.hostid));
        infoPCIBus.setText("PCI Bus: " + Integer.toString(data.card.bus));
        infoPCISlot.setText("PCI Slot: " +
                            Integer.toString(data.card.slot));
        infoDataslot.setText("Dataslot: " +
                             Integer.toString(data.card.dataslot));
        infoType.setText("Type: " +
                         HVData.HV_TYPE.getValue(data.module.type).toString());
        infoID.setText("ID: " + Integer.toString(data.module.ID));
        infoBusAddr.setText("Bus Addr: " +
                            Integer.toString(data.module.busaddr));
        infoNumChans.setText("# Channels: " +
                             Integer.toString(data.module.num_chans));
        infoHVMon.setText("HVMon: " +
                          Integer.toString(data.module.hvmon) + "V");
        infoNeg5V.setText("-5V: " + Integer.toString(data.module.pos5v));
        infoPos5V.setText("+5V: " + Integer.toString(data.module.neg5v));
        infoInterlock.setText("Interlock: " +
                              Integer.toString(data.module.interlock));
        infoILockDelay.setText("ILock delay: " +
                               Integer.toString(data.module.ilock_delay) +
                               "ms");
        infoILockDelayCnt.setText("ILock delay cnt: " +
                                  Integer.toString(data.module.
            ilock_delay_cnt) + "ms");
        infoMasterID.setText("Master ID: " +
                             Integer.toString(data.module.master_id));
        ...
        infoMasterChan.setText("Master Channel: " +
                               Integer.toString(data.module.master_chan +
                                                1));
        infoMasterTripDelay.setText("Master TripDelay: " +
                                    Integer.toString(data.module.
            master_trip_delay) + "ms");
        infoState.setText("State: " +
                          HVData.HV_STATE.getValue(data.module.state).desc());
        infoStatus.setText("Status: " +
                           HVData.HV_STATE.getValue(data.module.status).desc());
        infoPolicy.setText("Policy: 0x" +
                           Integer.toHexString(data.module.policy));
        infoPrimaryStatus.setText("Primary Status: " +
                           HVData.HV_STATE.getValue(data.module.pr_status).desc());

        setTitle(HostID + ":" + HVCard + ":" + BusAddr + " " +
                 HVData.HV_TYPE.getValue(data.module.type).toString() +
                 " ID#" + Integer.toString(data.module.ID)
            );

        setToolTipText(HostID + ":" + HVCard + ":" + BusAddr + " " +
                       HVData.HV_TYPE.getValue(data.module.type).toString() +
                       " ID#" + Integer.toString(data.module.ID)
            );

        chdata = data.module.getRealChanValues();
        HVchannel ch = new HVchannel();

        int vseries = VmonPlotsPanel.getNumSeries();
        if (vseries != chdata.length) {
          VmonPlotsPanel.setNumSeries(chdata.length);
          vseries = chdata.length;
        }
        ...
        if (iseries != chdata.length) {
          ImonPlotsPanel.setNumSeries(chdata.length);
          iseries = chdata.length;
        }
    ...
    private void jbInit() throws Exception {
      ...
      this.addInternalFrameListener(new HVModuleFrame_this_internalFrameAdapter(this));
      pack();
      setSize(800, 800);
      ...
      modelHVCard = new SpinnerNumberModel(HVCard, 0, 20, 1);
      ...
      spHostID.addChangeListener(new HVModuleFrame_spinners_changeAdapter(this));
      spHVCard.addChangeListener(new HVModuleFrame_spinners_changeAdapter(this));
      spBusAddr.addChangeListener(new HVModuleFrame_spinners_changeAdapter(this));
      ...
      spTimerDelay.addChangeListener(new
                                     HVModuleFrame_spTimerDelay_changeAdapter(this));
      lblTimerDelay.setText("Update (sec):");
      cbTimerCtrl.setSelected(true);
      cbTimerCtrl.setText("Update On/Off");
      cbTimerCtrl.addActionListener(new HVModuleFrame_cbTimerCtrl_actionAdapter(this));
    ...
    public void updateTitle() {
      setTitle(HostID + ":" + HVCard + ":" + BusAddr);
//    setTitle("HV" + openFrameCount + " - HostID=" + HostID+ ", HVCard=" + HVCard +", BusAddr="+BusAddr);
    }
  ...
  class HVModuleFrame_cbTimerCtrl_actionAdapter
      implements ActionListener {
    private HVModuleFrame adaptee;
    HVModuleFrame_cbTimerCtrl_actionAdapter(HVModuleFrame adaptee) {
  ...
  class HVModuleFrame_spTimerDelay_changeAdapter
      implements ChangeListener {
    private HVModuleFrame adaptee;
    HVModuleFrame_spTimerDelay_changeAdapter(HVModuleFrame adaptee) {
      this.adaptee = adaptee;
    }
  ...
  class HVModuleFrame_this_internalFrameAdapter
      extends InternalFrameAdapter {
    private HVModuleFrame adaptee;
    HVModuleFrame_this_internalFrameAdapter(HVModuleFrame adaptee) {
      this.adaptee = adaptee;
    }
  ...
  class HVModuleFrame_spinners_changeAdapter
      implements ChangeListener {
    private HVModuleFrame adaptee;
    HVModuleFrame_spinners_changeAdapter(HVModuleFrame adaptee) {
      this.adaptee = adaptee;
    }
