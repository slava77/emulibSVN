...
// Logger hvcomdebug  = Logger::getRoot();
...
int main(int argc, char **argv)
{
  ...
  */
  ...
  SharedAppenderPtr append_8(new RollingFileAppender(HV_LOGSDIR+"/hvComDebug.log", 1024*1024, 10));
  append_8->setName("HVComDebug");
  append_8->setLayout( std::auto_ptr<Layout>(new SimpleColoredLayout(false)) );
  Logger::getInstance("hvComDebug").addAppender(append_8);

  hvcomdebug = Logger::getInstance("hvComDebug");
  ...
  stringstream devfn;
  devfn.clear();
  devfn << "/dev/" << DRV_MODULE_NAME << devn;

  fd = open(devfn.str().c_str(),O_RDWR);
  if (fd<0)
    {
