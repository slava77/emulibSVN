...
int ResetCOM(int port)
{
  ...
  if ((res = CloseCOM(port)) < 0)
    {
      LOG4CPLUS_WARN( hvcomdebug,"ResetCOM: Unable to close COM" << port);
    }
  if ((res = InitCOM(port)) < 0)
    {
      LOG4CPLUS_WARN( hvcomdebug, "ResetCOM: Unable to open COM" << port);
    }
  ...
  if (strstr (buf,"!"))
    {
      ...
      if (SendHVCmd(port, addr, "STS") <0)
        {
          // printf("error sending STS 2nd time\n");
          return -1;
        };
      if (ComRdTerm(port, buf, sizeof(buf), 13) <0)
        {
          // printf("error receiving STS 2nd time\n");
          return -1;
        };
    }
  // *ctrlstatus = 1;
