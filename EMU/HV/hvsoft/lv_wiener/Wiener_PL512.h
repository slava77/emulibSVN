...
class Wiener_PL512 {
  ...
  int SetValue(const char *Community, u_char Type, const void *Val, int ValLen,
                                          const char *PName, int Channel = 0) {
    int ind = dict.GetIndex(PName);
...
//      printf("%ssnmp_open successful\n", Time_Stamp());
 ...
 public:
  Wiener_PL512(const char *IPAddress = NULL) : IP(NULL), Value(NULL),
                             ...
                             Timeout(SNMP_DEFAULT_TIMEOUT) { init(IPAddress); }

  ~Wiener_PL512() { DeleteAll(); }
...
//          print_variable(vars->name, vars->name_length, vars);
  ...
  inline int GetUpTime() const { return GetTimeticks("UpTime"); }
  ...
  inline int GetChannelTemperatureStatus(int Channel, char *OutValue = NULL)
     const { return GetChEnum("ChannelTemperatureStatus", Channel, OutValue); }
  inline int GetChannelOutputSwitch(int Channel, char *OutValue = NULL)
          const { return GetChEnum("ChannelOutputSwitch", Channel, OutValue); }
  ...
  inline int GetOperatingTime() const { return GetInteger("OperatingTime"); }
