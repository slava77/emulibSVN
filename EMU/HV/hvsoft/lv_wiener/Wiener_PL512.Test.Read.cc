...
int main() {
  Wiener_PL512 PS("UFCMSHVLVPS2.cern.ch");
...
//  PS.PrintValues();
  ...
  printf("Channel measured terminal voltages:");
  for (int i=1; i<=PS.GetNumberOfChannels(); i++) {
    fval = PS.GetChannelMeasuredTerminalVoltage(i, Text);
    if (i > 1) printf(",");
    printf(" %d : %f %s", i, fval, Text);
  }
  printf("\n");
  ...
  printf("Channel max power:");
  for (int i=1; i<=PS.GetNumberOfChannels(); i++) {
    fval = PS.GetChannelMaxPower(i, Text);
    if (i > 1) printf(",");
    printf(" %d : %f %s", i, fval, Text);
  }
  printf("\n");
  ...
  printf("================================================== Timeout test.\n");
  for (int timeout=250000; timeout<=1000000; timeout+=250000)
