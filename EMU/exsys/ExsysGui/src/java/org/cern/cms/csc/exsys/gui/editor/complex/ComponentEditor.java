    ...
    public void componentInputValueChanged(ValueChangeEvent event) throws IllegalAccessException, InvocationTargetException, ComponentNotFoundException {
        ...
        if (event.getComponent() instanceof SelectInputText) {
            ...
            if (newWord != null && newWord.length() > 0) {
                ...
                for (GComponent gc: getGenericDao().getGOntologyDao().getGComponentsByNameMatches(newWord, getComponentClassLimit(), MATCHES_TO_DISPLAY)) {
                ...
                if (autoComplete.getSelectedItem() != null) {
                    Component c = getGenericDao().getOntologyDao().getComponentByName(((GComponent) autoComplete.getSelectedItem().getValue()).getName());
                    setValue(c);
                }
                    ...
                    if (componentMatches != null) {
                        ...
                        for(int i = 0, max = componentMatches.size(); i < max; i++){
                            ...
                            if (tmp.getName().compareToIgnoreCase(newWord) == 0) {
                                setValue(getGenericDao().getOntologyDao().getComponentByName(tmp.getName()));
                                break;
                            }
                        }
                    }
