...
void EmuPeripheralCrateConfig::RTRGhigh(xgi::Input * in, xgi::Output * out ) 
  ...
  if(name != cgi.getElements().end()) {
    dmb = cgi["dmb"]->getIntegerValue();
    std::cout << "Read TDO status from ODMB: " << dmb << std::endl;
    DMB_ = dmb;
  }
  //
//  OutputDMBTests[dmb][current_crate_] << "Toggle Random Trigger High" << std::endl ;
  ...
  DAQMB * thisDMB = dmbVector[dmb];
  //
  CFEBDataOut_=(thisDMB->test11())&0xFFFF;
...
// modified for ODMB-PPIB test
  ...
  DAQMB * thisDMB = dmbVector[dmb];
     ...
     if(value2 != cgi.getElements().end()) 
       pulses = atoi(cgi["CFEBDataIn"]->getValue().c_str());

     CFEBDataIn_=sig;

     std::cout << "generating signal: " << sig << std::endl;
     std::cout << "number of pulses: " << pulses << std::endl;
     if(sig>=0 && sig<=25)
     {
        std::cout << "Pulsing start..." << std::endl;
        thisDMB->virtex6_writereg(sig, pulses);
  ...
  *out << cgicc::legend("CFEB ID Readback").set("style","color:blue") << std::endl ;
  ...
  } else if(hversion==2)
  {
    ...
    *out << buf;
    ...
    {
      unsigned id=0x4424A093;
      int bad=0;
      for(int i=0; i<1000; i++)
        if(id!=thisDMB->febfpgaid(*cfebItr)) bad++;
      if(bad>0) *out << "-----> IDcode check with error(s): " << bad << cgicc::br();
      else *out << "-----> IDcode check passed." << cgicc::br();
    }
/*
//--->> extra: DNA and ADC
    unsigned long dna;
    int i=thisDMB->dcfeb_dna(*cfebItr, &dna);
    *out << cgicc::br() << "DNA: " << std::hex << dna << std::dec << cgicc::br();

     int j;
   for(int i=0;i<7; i++)
   {
     j=thisDMB->dcfeb_adc(*cfebItr, i);
     *out <<  cgicc::br() << "ADC: " << i << " " << j << cgicc::br();
   }
//---> end extra
*/
  }
...
void EmuPeripheralCrateConfig::DMBUtils(xgi::Input * in, xgi::Output * out ) 
  ...
  *out << cgicc::h1(Name);
  ...
  //
if(thisDMB && thisDMB->GetHardwareVersion()>2)
{

  std::vector <std::string> FuncName; 
  char Name[100];
  FuncName.push_back("Hard Reset");
  FuncName.push_back("All JTAG signals (9)");
  FuncName.push_back("All TTC signals (13)"); 
  FuncName.push_back("TCK 1"); 
  FuncName.push_back("TCK 2"); 
  FuncName.push_back("TCK 3"); 
  FuncName.push_back("TCK 4"); 
  FuncName.push_back("TCK 5"); 
  FuncName.push_back("TCK 6"); 
  FuncName.push_back("TCK 7"); 
  FuncName.push_back("TMS"); 
  FuncName.push_back("TDI"); 
  FuncName.push_back("L1A MATCH 1"); 
  FuncName.push_back("L1A MATCH 2"); 
  FuncName.push_back("L1A MATCH 3"); 
  FuncName.push_back("L1A MATCH 4"); 
  FuncName.push_back("L1A MATCH 5"); 
  FuncName.push_back("L1A MATCH 6"); 
  FuncName.push_back("L1A MATCH 7"); 
  FuncName.push_back("L1A"); 
  FuncName.push_back("RESYNC"); 
  FuncName.push_back("BC0"); 
  FuncName.push_back("REPRGEN"); 
  FuncName.push_back("EXTPLS"); 
  FuncName.push_back("INJPLS");

  *out << cgicc::fieldset().set("style","font-size: 11pt; font-family: arial;") << std::endl;
  *out << cgicc::legend("ODMB-PPIB Test Functions").set("style","color:blue") << std::endl ;
  //
  ...
  *out << cgicc::h1("PPIB production test");

  *out << cgicc::h2("1. choose signal");
  *out << cgicc::h2("2. type in number of pulses");
  *out << cgicc::h2("3. press [Start Pulses] button");
  ...
  *out << "Number of Pulses: " << std::endl;
  sprintf(sbuf, "%d", 100000);
  *out << cgicc::input().set("type","text").set("value",sbuf).set("name","CFEBDataIn") << std::endl ;
  *out << cgicc::input().set("type","hidden").set("name","dmb").set("value",dmbstring) << std::endl ;          
  *out << cgicc::input().set("type", "submit")
    .set("name", "command")
    .set("value", "Start Pulses") << cgicc::br() << std::endl;
   if(CFEBDataIn_>=0 && CFEBDataIn_<FuncName.size())
     *out << "  <<====== last run : " << FuncName[CFEBDataIn_] << std::endl;
    
  *out << cgicc::form() << cgicc::br() << std::endl;
  //End select signal

  std::string readbackTDO = toolbox::toString("/%s/RTRGhigh",getApplicationDescriptor()->getURN().c_str());
  *out << cgicc::form().set("method","GET").set("action",readbackTDO) << std::endl ;
  *out << cgicc::input().set("type","submit").set("value","Read Status") << std::endl ;
  sprintf(sbuf,"%d",dmb);
  *out << cgicc::input().set("type","hidden").set("value",sbuf).set("name","dmb");
  *out << cgicc::form() << std::endl ;
  //
  *out << cgicc::br();
  *out << "  ======> data read back : " << cgicc::br();
  *out << "<pre>" << std::endl;
  sprintf(sbuf, "x-----TDO-------x-----DONE----- ");
  *out << sbuf << cgicc::br();
  sprintf(sbuf, "x 7 6 5 4 3 2 1 x 7 6 5 4 3 2 1 ");
  *out << sbuf << cgicc::br();
  sprintf(sbuf, "0 0 0 0 0 0 0 0 x 0 0 0 0 0 0 0 ");
  unsigned a=CFEBDataOut_;
  for(int i=0; i<15; i++) 
  { 
     if(a&(1<<i)) sbuf[(15-i)*2]++;
  }
  *out << sbuf << cgicc::br();
  *out << "</pre>" << std::endl;
    //
