...
// when = 0  send vme packet LATER; 
//      = 1  send vme packet NOW;   
   int tdi_adr=0x5a+(theSlot<<19), tdo_adr=0x58+(theSlot<<19);
   int ctrl_adr=0x5c+(theSlot<<19);

  int DEBUG=0;
  int i, j, k, bit, cnt8;
  int TDI_=8;
  const unsigned short int TCK= dev&0x7F;
  const unsigned short int TMS=(1<<7);
  unsigned short int ival, d, dd, pvme=0;
  unsigned int ptr;
  unsigned char *rcv2, bdata, *data, mytmp[MAXLINE], mask=dev&0x7F;
  int buff_mode;
 ...
 //
   ...
   dd=0x4000;
   theController->VME_controller(1,ctrl_adr,&dd,rcv);
  ...
  for(i=0;i<cnt;i++) 
  {
    ival = bdata&0x01;
     ...
     if(ird){
       ...
       theController->VME_controller(0,tdo_adr,&dd,rcv);
     }
  ...
  if(ird)
  {
    ...
    // Use cnt8 instead of cnt, to make sure the bits are in right place.
     ...
     for(i=0;i<cnt8;i++)
     {
       if(i<cnt) bdata |= ((mytmp[2*i+1]&mask)?0x80:0); 
