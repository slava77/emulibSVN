
/* This is test_functions.h
 * It contains function declarations
 */

int    test_5_init(void);
int    test_6_init(void);
int    test_7_init(void);
int    test_8_init(void);
int    test_9_init(void);
int    test_10_init(void);
int    test_11_init(void);
int    test_12_init(void);
int    test_13_init(void);
int    test_14_init(void);
int    test_15_init(void);
int    test_16_init(void);
int    test_17_init(void);
int    test_18_init(void);
int    test_19_init(void);
int    test_20_init(void);
int    test_21_init(void);
int    test_22_init(void);
int    test_23_init(void);
int    test_24_init(void);
int    test_25_init(void);
int    test_26_init(void);
int    test_27_init(void);
int    test_28_init(void);
int    test_29_init(void);
int    test_16cpp_init(void);

