/*
 * test_16cpp.cpp (in V1.2  test_11.c)
...
int test_16cpp_begin(int pass);
int test_16cpp_event(int pass);
int test_16cpp_end(int pass);
int test_16cpp_finish(void);
...
char test_16cpp_file_el[]="/test_results/temp/test_16cpp_01.results";
char test_16cpp_file_bad_el[]="/test_results/temp/test_16cpp_01.bad";
char test_16cpp_file_his_el[]="/test_results/temp/test_16cpp_01.his";

/*******************************************************************************
   Define functions and parameters for FAST site test 16
*******************************************************************************/
static int nstrips, ncfebs;
extern "C" int test_16cpp_init(void)
{
  int l;

  test_begin  = test_16cpp_begin;
  test_event  = test_16cpp_event;
  test_end    = test_16cpp_end;
  test_finish = test_16cpp_finish;
  ...
  switch (csc_type)
    {
    case 0+7:  nstrips = 48, ncfebs = 3; break;
    case 1+7:  nstrips = 64; ncfebs = 4; break;
  ...
  if (pass == 0)
    {
      ...
      filename_bad = (char*)malloc(strlen(datadir_translation) +
                                   strlen(test_16cpp_file_bad_el) + 1 );
      if (filename_bad == NULL)
        {
          printf("malloc() failed on filename.\n");
          return 1;
        }
      sprintf(filename,"%s%s",datadir_translation,test_16cpp_file_el);
      sprintf(filename_bad,"%s%s",datadir_translation,test_16cpp_file_bad_el);
  ...
  if (pass == 0)
    {
      ...
      file_his_name = (char*)malloc(strlen(datadir_translation) +
                                    strlen(test_16cpp_file_his_el) + 1);
      if (file_his_name == NULL)
      ...
      else
        {
          sprintf(file_his_name,"%s%s",
                  datadir_translation,test_16cpp_file_his_el);
