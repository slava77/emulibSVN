...
int test_16_init(void) 
{
    ...
    padc_diff     = malloc (sizeof(float) * NLAYER * nstrips);
    padc_diff_err = malloc (sizeof(float) * NLAYER * nstrips);
...
/* Initializations */
	...
	filename_bad = malloc(strlen(datadir_translation) + 
...
/* Book histograms */
	HBOOK1(1, "timesample with minimum ADC", 16, 0., 16., 0.);
	HBOOK1(2, "timesample with maximum ADC", 16, 0., 16., 0.);
    }
			...
			if (upevt_.active_febs[ifeb] > 0) 
			{
				for (ilayer = 0; ilayer < NLAYER; ilayer++) 
				{
					...
					for (istrip = 16 * ifeb; istrip < 16 + 16 * ifeb; istrip++) 
					{
						...
						HFILL(1, (float)jmin, 0., 1.);
						HFILL(2, (float)jmax, 0., 1.);
					}
				}
			}
		}
		...
		for (ifeb = 0; ifeb < ncfebs; ifeb++) 
		{
			if (upevt_.active_febs[ifeb] > 0) 
			{
				for (ilayer = 2*layerpair; ilayer < 2*(layerpair+1); ilayer++) 
				{
					for (istrip = 16 * ifeb; istrip < 16 + 16 * ifeb; istrip++) 
					{
						// to filter out zero events
						if(upevt_.sca[ilayer][istrip][tsmax[1]]==0)continue; 
						x = 0.;
						for (i = tsmax[0]; i<=tsmax[1]; i++)
						{
							x += upevt_.sca[ilayer][istrip][i] -
								upevt_.sca[ilayer][istrip][0];
						}
						sum[ilayer][istrip] += x;
